...
from causalm_utils import WORDPIECE_PREFIX, MASK_TOKEN, CLS_TOKEN, SEP_TOKEN
# from causalm_constants import SENTIMENT_DOMAINS
from constants import BERT_PRETRAINED_MODEL
from filename_formats import full_pivots_csv_format
...
class DocumentDatabase:
    ...
    def __getitem__(self, item):
        if self.reduce_memory:
            ...
            return self.document_ids[item], self.documents[item], self.documents_treatment_labels[item], \
                   self.documents_control_labels[item]
...
def generate_data_for_domain(args, domain):
    """
    :param args: A namespace containing:
        - pivots_path
        - output_dir
        - bert_model
        - do_lower_case (unused) (determined from bert_model)
        - do_whole_word_mask
        - reduce_memory
        - num_workers
        - epochs_to_generate
    ...
    try:
        ...
        bert_model = BERT_PRETRAINED_MODEL

    tokenizer = BertTokenizer.from_pretrained(bert_model, do_lower_case=bool(bert_model.endswith("uncased")))
    vocab_list = list(tokenizer.vocab.keys())

    treatment_topic = args.treated_index
    control_topic = args.control_index

    treatment_column = f"pivot{treatment_topic}"
    control_column = f"pivot{control_topic}"
    ...
    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        ...
        for dataset in ("train", "dev"):
            DATASET_FILE = f"{args.pivots_path}/{full_pivots_csv_format(dataset, args.n_pivots, args.concept_type, args.shap)}"
            if not os.path.isfile(DATASET_FILE):
                # TODO: insert a dataset creation call
                raise NotImplementedError('Not implemented lazy dataset creation inside data pregeneration yet :(\n'
                                          'File {} not found.'.format(DATASET_FILE))
                pass
            df = pd.read_csv(DATASET_FILE, header=0, encoding='utf-8',
                             usecols=["id", "review", treatment_column, control_column])\
        ...
        for unique_id, doc, treatment_label, control_label in tqdm(
            ...
            if doc:
                docs.add_document(unique_id, doc, treatment_label,
                                  control_label)  # If the last doc didn't end on a newline, make sure it still gets added
        if len(docs) <= 1:
