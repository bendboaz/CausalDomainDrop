...
from bert_pivots_finetune import BertForPivotTreatControlPreTraining, \
    BertForPivotTreatPreTraining
from bert_text_dataset import BertTextDataset
from constants import LOGGER_NAME, BERT_PRETRAINED_MODEL
from constants import PRETRAIN_NUM_WORKERS
...
class PregeneratedDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs, reduce_memory=False):
        ...
        if reduce_memory:
            ...
            lm_label_ids = np.full(shape=(num_samples, seq_len), dtype=np.int32,
                                   fill_value=BertTextDataset.MLM_IGNORE_LABEL_IDX)
        ...
        self.logger.debug(f"Loading training examples for epoch {epoch}")
...
def pretrain_on_domain(args):
    ...
    using_parallel = not args.no_cuda and n_gpu > 1
    LOGGER.info("device: {} n_gpu: {}, parallel training: {}, distributed training: {}, 16-bits training: {}".format(
        device, n_gpu, using_parallel, bool(args.local_rank != -1), args.fp16))
    ...
    tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=bool(args.bert_model.endswith('uncased')))
    ...
    # Prepare model
    if args.control:
        model = BertForPivotTreatControlPreTraining.from_pretrained(args.bert_model)
    else:
        model = BertForPivotTreatPreTraining.from_pretrained(args.bert_model)
    if args.fp16:
        model.half()
    model.to(device)
    if args.local_rank != -1:
    ...
    is_main_device = args.local_rank == -1 or torch.distributed.get_rank() == 0
    global_step = 0
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.control:
                    outputs = model(input_ids=input_ids, attention_mask=input_mask, masked_lm_labels=lm_label_ids,
                                    pivot_treat_label=treatment_label, pivot_control_label=control_label)
                else:
                    outputs = model(input_ids=input_ids, attention_mask=input_mask, masked_lm_labels=lm_label_ids,
                                    pivot_treat_label=treatment_label)
                ...
                for i in range(unique_id.size(0)):
                    ...
                    if args.control:
                        ...
                        loss_dict["total_loss"].append(
                            mlm_loss[i].item() + treatment_loss[i].item() + control_loss[i].item())
                    else:
                        loss_dict["total_loss"].append(mlm_loss[i].item() + treatment_loss[i].item())
        # Save a trained model
        if epoch < num_data_epochs and is_main_device:
            LOGGER.info("** ** * Saving fine-tuned model ** ** * ")
            module = model
            if isinstance(model, torch.nn.DataParallel) or (args.local_rank != -1 and isinstance(model, DDP)):
                module = model.module
            epoch_output_dir = args.output_dir / f"epoch_{epoch}"
            epoch_output_dir.mkdir(parents=True, exist_ok=True)
            module.save_pretrained(epoch_output_dir)
