...
class BertPivotTreatPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        # self.transform = BertPredictionHeadTransform(config)
        self.pooler = BertPivotTreatPredictionHead.masked_avg_pooler
...
class BertPivotControlPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.pooler = BertPivotTreatPredictionHead.masked_avg_pooler
        # self.pooler = HAN_Attention_Pooler_Layer(config.hidden_size)
...
class BertPivotTreatPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertLMPredictionHead(config)
        self.pivot_treat_prediction = BertPivotTreatPredictionHead(config)

    def forward(self, sequence_output, sequence_mask):
        lm_prediction_scores = self.predictions(sequence_output)
        pivot_treat_prediction_score = self.pivot_treat_prediction(sequence_output, sequence_mask)
        return lm_prediction_scores, pivot_treat_prediction_score


class BertForPivotTreatPreTraining(BertPreTrainedModel):
    r"""
    ...
    def __init__(self, config):
        ...
        self.cls = BertPivotTreatPreTrainingHeads(config)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        lm_prediction_scores, pivot_treat_prediction_score = self.cls(sequence_output, attention_mask)

        outputs = (lm_prediction_scores, pivot_treat_prediction_score,) + outputs[
                                                                          2:]  # add hidden states and attention if they are here

        if masked_lm_labels is not None and pivot_treat_label is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            masked_lm_loss = loss_fct(lm_prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
            pivot_treat_loss = loss_fct(pivot_treat_prediction_score.view(-1, 2), pivot_treat_label.view(-1))
            loss = masked_lm_loss + pivot_treat_loss
            ...
            outputs = (loss,
                       torch.stack([loss_fct_per_sample(lm_prediction_scores.view(-1, self.config.vocab_size),
                                   ...
                                   .view_as(masked_lm_labels)[i, :].masked_select(masked_lm_labels[i, :] > -1)
                                   .mean()
                                    for i in range(masked_lm_labels.size(0))]),
                       loss_fct_per_sample(pivot_treat_prediction_score, pivot_treat_label),) + outputs

        return outputs  # (loss), prediction_scores, seq_relationship_score, (hidden_states), (attentions)


class BertPivotTreatControlPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertLMPredictionHead(config)
        self.pivot_treat_prediction = BertPivotTreatPredictionHead(config)
        self.pivot_control_prediction = BertPivotControlPredictionHead(config)

    def forward(self, sequence_output, sequence_mask):
        lm_prediction_scores = self.predictions(sequence_output)
        pivot_treat_prediction_score = self.pivot_treat_prediction(sequence_output, sequence_mask)
        pivot_control_prediction_score = self.pivot_control_prediction(sequence_output, sequence_mask)
        return lm_prediction_scores, pivot_treat_prediction_score, pivot_control_prediction_score


class BertForPivotTreatControlPreTraining(BertPreTrainedModel):
    r"""
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            Labels for computing the masked language modeling loss.
            Indices should be in ``[-1, 0, ..., config.vocab_size]`` (see ``input_ids`` docstring)
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.cls = BertPivotTreatControlPreTrainingHeads(config)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, masked_lm_labels=None,
                pivot_treat_label=None, pivot_control_label=None):
        outputs = self.bert(input_ids,
                            attention_mask=attention_mask,
                            token_type_ids=token_type_ids,
                            position_ids=position_ids,
        ...
        sequence_output, pooled_output = outputs[:2]
        lm_prediction_scores, pivot_treat_prediction_score, pivot_control_prediction_score = self.cls(sequence_output,
                                                                                                      attention_mask)

        outputs = (lm_prediction_scores, pivot_treat_prediction_score, pivot_treat_prediction_score,) + outputs[
                                                                                                        2:]  # add hidden states and attention if they are here

        if masked_lm_labels is not None and pivot_treat_label is not None and pivot_control_label is not None:
            loss_fct = CrossEntropyLoss(ignore_index=BertTextDataset.MLM_IGNORE_LABEL_IDX)
            masked_lm_loss = loss_fct(lm_prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
            pivot_treat_loss = loss_fct(pivot_treat_prediction_score.view(-1, 2), pivot_treat_label.view(-1))
            pivot_control_loss = loss_fct(pivot_control_prediction_score.view(-1, 2), pivot_control_label.view(-1))
            loss = masked_lm_loss + pivot_treat_loss + pivot_control_loss
            ...
            outputs = (loss,
                       torch.stack([loss_fct_per_sample(lm_prediction_scores.view(-1, self.config.vocab_size),
                                   ...
                                   .mean()
                                    for i in range(masked_lm_labels.size(0))]),
                       loss_fct_per_sample(pivot_treat_prediction_score, pivot_treat_label),
                       loss_fct_per_sample(pivot_control_prediction_score, pivot_control_label),) + outputs
